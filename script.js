...
import { setupRatingSystem } from './rating.js'; import { getDatabase, ref, push, onValue } from "https://www.gstatic.com/firebasejs/9.22.2/firebase-database.js"; import { initializeApp } from "https://www.gstatic.com/firebasejs/9.22.2/firebase-app.js";
...
const modal = document.getElementById("comment-modal"); const commentList = document.getElementById("comment-list"); const closeBtn = document.getElementById("close-comments"); const commentForm = document.getElementById("comment-form");
...
function checkVideoVisibility() { document.querySelectorAll("video").forEach(video => { const rect = video.getBoundingClientRect(); const videoHeight = rect.height; const scrolledOut = rect.bottom < window.innerHeight - (videoHeight * 0.2); if (scrolledOut && !video.paused) { video.pause(); } }); } window.addEventListener("scroll", checkVideoVisibility);

// Comment Modal Handling 
document.body.addEventListener("click", e => { if (e.target.classList.contains("comment-btn")) { activePostId = e.target.dataset.postid; loadComments(activePostId); modal.classList.remove("hidden"); } });

closeBtn.addEventListener("click", () => { modal.classList.add("hidden"); commentList.innerHTML = ""; });

// Store user ID in localStorage 
function getOrCreateUserName() { let userId = localStorage.getItem("user-id"); let userName = localStorage.getItem("user-name"); if (!userId || !userName) { userName = prompt("Enter your name:").trim(); if (!userName) return null; userId = crypto.randomUUID(); localStorage.setItem("user-id", userId); localStorage.setItem("user-name", userName); } return userName; }

commentForm.addEventListener("submit", e => { e.preventDefault(); const text = document.getElementById("comment-text").value.trim(); const name = getOrCreateUserName(); if (!name || !text) return;

const commentRef = ref(db, comments/${activePostId}); push(commentRef, { name, text, timestamp: Date.now() });

commentForm.reset(); });

function loadComments(postId) { const commentRef = ref(db, comments/${postId}); onValue(commentRef, snapshot => { commentList.innerHTML = ""; const comments = snapshot.val(); if (comments) { const sorted = Object.values(comments).sort((a, b) => a.timestamp - b.timestamp); sorted.forEach(c => { const div = document.createElement("div"); div.innerHTML = <strong>${c.name}</strong><p>${c.text}</p><hr/>; commentList.appendChild(div); }); } else { commentList.innerHTML = "<p>No comments yet.</p>"; } }); }
